...
def hybrid_model_config():
    """Create a ModelConfig for Qwen3.5 hybrid model."""
    return ModelConfig(
...
@pytest.fixture
def hybrid_vllm_config(hybrid_model_config, hybrid_cache_config):
    """Create a VllmConfig for Qwen3.5 hybrid model."""
    return VllmConfig(
        model_config=hybrid_model_config,
        cache_config=hybrid_cache_config,
    )


def test_update_block_size_for_backend_increases_block_size(
    hybrid_model_config, hybrid_cache_config
):
    ...
    # The block_size may have been updated during VllmConfig initialization
...
def test_update_block_size_for_backend_sets_mamba_padding(hybrid_vllm_config):
    ...
    # mamba_page_size_padded should be set (during VllmConfig initialization)
    ...
    attn_page_size = FullAttentionSpec(
        ...
        num_kv_heads=model_config.get_num_kv_heads(hybrid_vllm_config.parallel_config),
